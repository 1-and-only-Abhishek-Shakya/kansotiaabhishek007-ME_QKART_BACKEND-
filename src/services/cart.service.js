...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  if (product == null) {
    // res
    //   .status(httpStatus.BAD_REQUEST)
    //   .send("Product doesn't exist in database");
  ...
  if (productIndex == -1) {
    // res.status(httpStatus.BAD_REQUEST).send("Product not in cart");
...
const deleteProductFromCart = async (user, productId) => {
  ...
  if (!userCart) {
    ...
    const result = userCart.cartItems.find((item) => {
    ...
    if (!result) {
      ...
      const filterData = userCart.cartItems.filter((items) => {
        if (items._id == result._id) {
          return false;
        }
        return true;
      });
      userCart.cartItems = filterData;
    }
  }
...
const checkout = async (user) => {
  ...
  const hasSetNonDefaultAddress = await user.hasSetNonDefaultAddress(user.email);
